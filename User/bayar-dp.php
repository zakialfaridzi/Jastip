...
if (strlen($_SESSION['jastip'] == 0)) {
    ...
    if (isset($_GET['acc'])) {
        $idtr = $_GET['acc'];
        $acc = mysqli_query($con, "UPDATE tbltransaksi set status = 'in process', tanggal_terima=NULL where IDtransaksi='$idtr'");
        if ($acc) {
    ...
    if (isset($_POST['update'])) {
        ...
        $idtr = $_GET['viewid'];
        $upd = mysqli_query($con, "UPDATE tbltransaksi set keterangan='$ket', status='cancelled' where IDtransaksi='$idtr'");
        if ($upd) {
            echo "<script>alert('Entry Pesanan telah dibatalkan');</script>";
            echo "<script>window.location.href ='laporan-pembelian.php'</script>";
        }
    }
...
<head>
    ...
    <title>HelloWorld - Bayar DP</title>
    ...
    while ($row = mysqli_fetch_array($ret)) {?>
                            <table border="1" class="table table-bordered mg-b-0">
                                ...
                                <tr>
                                    ...
                                    <td><?php echo "Rp " . number_format(($row['harga_total']*0.5), 2, ',', '.'); ?></td>
                                </tr>
                                <tr>
                                    <th>Nomor Rekening Penjasa</th>
                                    <td><?php echo $row['norek']; ?></td>
                                </tr>
                            </table>
                            <form method="get">
                            <button type="submit" name="acc" class="btn btn-primary"  value="<?php echo $_GET['viewid']; ?>">Bayar DP</button>
                            </form>
                            <form method="post">
                            <button type="button" class="btn btn-danger" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">Batalkan</button>
